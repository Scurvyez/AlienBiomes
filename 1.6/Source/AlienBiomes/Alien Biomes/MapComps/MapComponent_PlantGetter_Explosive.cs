...
{
    public class MapComponent_PlantGetter_Explosive : MapComponent
    {
        ...
        public MapComponent_PlantGetter_Explosive(Map map) : base(map) { }
        ...
        private static int ComputeSelectionSignature()
        {
            ...
            for (int i = 0; i < selectedPlants.Count; i++)
            {
                ...
                hash = (hash * 31) ^ (comp.Exploded ? 1 : 0);
            }
            return hash;
        }
        ...
        private void RebuildSelectionHighlightCache()
        {
            ...
            for (int i = 0; i < _selectedPlants.Count; i++)
            {
                ...
                var comp = plant.TryGetComp<Comp_Explosive>();
                if (comp == null) continue;
                if (comp.Exploded) continue;

                var props = (CompProperties_Explosive)comp.props;

                _highlightColor = props.triggerRadiusColor;

                if (props.triggeredDamageDef != null && plant.Growth >= props.triggerGrowthThreshold)
                {
